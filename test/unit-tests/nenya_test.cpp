...
using string = nenya::strong_type<std::string, tags::string_t>;
using integer = nenya::strong_type<int, tags::integer_t>;
using boolean = nenya::strong_type<bool, tags::boolean_t>;

template <typename T>
using maybe = nenya::strong_type<std::optional<T>, tags::maybe_t>;
...
TEST(Nenya, SerializeInteger)
{
  {
    std::ofstream stream{"integer.bin", std::ios::binary};
  ...
  {
    std::ifstream stream{"integer.bin", std::ios::binary};
...
TEST(Nenya, SerializeString)
{
  {
    std::ofstream stream{"string.bin", std::ios::binary};
    cereal::BinaryOutputArchive archive{stream};
  ...
  {
    std::ifstream stream{"string.bin", std::ios::binary};
    cereal::BinaryInputArchive archive{stream};

    string str;
    archive(str);

    ASSERT_EQ("foobar", str.get());
  }
}

TEST(Nenya, ConditionallyExplicit)
{
  using implicit_int = nenya::strong_type<int, struct implicit_int_t>;
  using explicit_int =
      nenya::strong_type<int, struct explicit_int_t, nenya::explicit_conversion>;
